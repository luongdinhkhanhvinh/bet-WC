...
router.get("/highscore", async (req, res) => {
  const columns = [
    { title: "Tên", orderBy: "name", orderDir: "asc" },
    { title: "Điểm", orderBy: "score", orderDir: "desc" },
    { title: "Thay đổi", orderBy: "score_change", orderDir: "desc" },
  ...
  res.render("highscore", {
    users: results.rows.filter((item) => item.id !== 1),
    columns,
    friends: onlyFriends,
